...
export class FountainCompletionProvider implements vscode.CompletionItemProvider {
	...
	provideTransitionCompletionItems(key: string, previousLineIsEmpty: boolean, position: vscode.Position): vscode.CompletionItem[] {
		...
		completes.push({ label: ">{=镜头交切=} ↓ (只含 以后 新场景)", filterText: key + '{=镜头交切=} ↓', insertText: previousLineIsEmpty ? '>{=镜头交切=} ↓' : '\n>{=镜头交切=} ↓', range: new vscode.Range(position.translate(0, -1), position), documentation: "插入镜头交切 (只含 以后 新场景)", sortText: "0j" });
		completes.push({ label: ">{#镜头交切#} ↓（含 前一.当前.以后 场景", filterText: key + '{#镜头交切#} ↓', insertText: previousLineIsEmpty ? '>{#镜头交切#} ↓' : '\n>{#镜头交切#} ↓', range: new vscode.Range(position.translate(0, -1), position), documentation: "插入镜头交切（含 前一.当前.以后 场景）", sortText: "0k" });
		completes.push({ label: ">{+镜头交切+} ↓ (含 当前.以后 场景）", filterText: key + '{+镜头交切+} ↓', insertText: previousLineIsEmpty ? '>{+镜头交切+} ↓' : '\n>{+镜头交切+} ↓', range: new vscode.Range(position.translate(0, -1), position), documentation: "插入镜头交切（含 当前.以后 场景）", sortText: "0k" });
		completes.push({ label: ">{-结束交切-} ↑", filterText: key + '{-结束交切-} ↑', insertText: previousLineIsEmpty ? '>{-结束交切-} ↑' : '\n>{-结束交切-} ↑', range: new vscode.Range(position.translate(0, -1), position), documentation: "插入结束交切", sortText: "0l" });
