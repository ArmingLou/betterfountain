import * as docxmaker from "./docxmaker";
import * as print from "../pdf/print";
import { FountainConfig, ExportConfig } from "../configloader";
import * as helpers from "../helpers";
import * as fliner from "../pdf/liner";
import * as vscode from "vscode";

//Creates the PDF, or returns stats if output path is "$STATS$"
export var GenerateDocx = async function (outputpath: string, config: FountainConfig, exportconfig: ExportConfig, parsedDocument: any, progress?: vscode.Progress<{ message?: string; increment?: number; }>): Promise<any> {
    ...
    while (current_index < parsedDocument.tokens.length) {
        ...
        if (current_token.type == "dual_dialogue_begin" || current_token.type == "dialogue_begin" || current_token.type == "dialogue_end" || current_token.type == "dual_dialogue_end" ||
            ...
            if (current_token.type == "section") {
                //on the next scene header, add an invisible section (for keeping track of sections when creating bookmarks and generating docx-side)
    ...
    //     var docxConfig = vscode.workspace.getConfiguration("fountain.docx");
    // }
    ...
    docx_options.print.synopsis.feed = docx_options.print.section.feed
